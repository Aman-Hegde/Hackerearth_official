...
const Leaderboard = () => {
  ...
  return (
    <div
      className={`min-h-screen pt-7 pb-10 px-4 sm:px-8 lg:px-12 transition-colors duration-700 ${isDark
    ...
    >
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8 px-2 sm:px-6 md:px-12">
          <div
            className={`inline-flex items-center space-x-3 backdrop-blur-xl border border-transparent rounded-full px-6 py-4 mt-4 shadow-lg ${isDark ? "bg-indigo-900/60 text-indigo-300" : "bg-indigo-200/90 text-indigo-900"
              }`}
          >
          ...
          <h1
            className={`mt-2 text-5xl sm:text-6xl font-extrabold leading-tight ${isDark ? "text-white" : "text-indigo-900"
              }`}
          >
        ...
        {/* Table Container */}
        <div
          className={`shadow-xl rounded-3xl bg-white dark:bg-gray-900 border ${isDark ? "border-gray-600" : "border-gray-300"
        ...
        >
          <table className="min-w-full border-collapse border-spacing-0">
            <thead
              className={`${isDark ? "bg-indigo-900" : "bg-indigo-200"
                } z-10`}
            >
              <tr>
                {["Name", "Total Score", "Time Taken"].map((header) => (
                  <th
                    key={header}
                    scope="col"
                    className={`px-2 sm:px-6 py-2 sm:py-3 text-left font-semibold text-sm sm:text-lg border-b ${isDark ? "border-indigo-700 text-indigo-300" : "border-indigo-300 text-indigo-700"
                      } select-none`}
                  >
                    {header}
                  </th>
            ...
            <tbody>
              {data.length === 0 ? (
                ...
                data.map((row, idx) => (
                  <tr
                    key={idx}
                    className={`transition-transform transform hover:scale-[1.02] cursor-pointer rounded-lg shadow-sm select-none ${isDark
                        ? idx % 2 === 0
                          ? "bg-indigo-900"
                          : "bg-indigo-800"
                        : idx % 2 === 0
